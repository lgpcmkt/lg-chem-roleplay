...
export const RoleplayRoom: React.FC<RoleplayRoomProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 font-sans h-[100dvh] relative overflow-hidden text-slate-800">
      ...
      <div className="px-3 py-2.5 flex items-center justify-between z-10 sticky top-0 bg-white border-b border-slate-200">
        <button onClick={handleExit} className="flex items-center p-2 -ml-1 rounded-xl hover:bg-slate-100 transition-colors text-slate-500 font-bold text-sm">
          <ArrowLeft className="w-5 h-5" />
        </button>

        <div className="flex items-center gap-2.5 animate-fadeIn">
          <div className={`relative w-11 h-11 rounded-full border border-slate-200 bg-white shadow-sm flex items-center justify-center overflow-hidden shrink-0 ${conversation.isSpeaking ? 'animate-pulse ring-2 ring-blue-100' : ''}`}>
            <img src="/images/doctor_z2.png" alt="doc" className="w-full h-full object-cover" />
          </div>
          <div className="flex flex-col items-center justify-center">
            <h2 className="font-black text-sm tracking-tight text-slate-800">고객</h2>
            <p className="text-[10px] font-bold text-blue-500 bg-blue-50 px-2 py-0.5 rounded-full mt-0.5">
              VS {scenario.title} 스위칭
            </p>
          </div>
        </div>

        <button onClick={onHome} className="p-2 -mr-1 rounded-full hover:bg-slate-100 transition-colors text-slate-500">
